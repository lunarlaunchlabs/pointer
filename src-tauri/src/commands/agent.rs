...
fn render_opencode_agent_prompt(
    ...
    if mode == ExecutionMode::Plan {
        out.push_str("\n\nPlan mode contract: inspect only the context needed to create an executable plan, then stop. Read the active/named file when relevant, directly related implementation files, nearby examples for additive work, likely tests/specs/fixtures, and project config for verification. Use only repository-relative paths returned by tools. If a read fails, list the parent and retry the exact relative path before finalizing. Do not assume a language, framework, package manager, or bug; compare claims to code you read. Do not edit, run shell commands, create todos, or delegate. Final format: Context read: exact paths; Assessment: evidence-based finding; Plan: concrete minimal steps; Verification: exact configured command. Keep under 180 words.");
        ...
        out.push_str("\n\nAgent implementation constraints: gather minimal evidence, make the smallest relevant edit, preserve unrelated code/assets, and verify with the narrowest configured repository command when available. If attached context already contains sufficient evidence, proceed to edits; re-read only files you will modify or files whose exact full contents are needed. For additive/edit tasks with clear target files and one analogous example visible, perform at most two extra reads before the first edit. Do not keep reading examples once one useful local pattern is visible. Do not install/update dependencies or run destructive git/deploy/database commands unless explicitly requested. Do not use one-off package executors such as npx, npm exec, pnpm dlx, yarn dlx, or bunx; prefer existing project scripts or report the exact blocker. After edits, final response must concisely list changed files and the real verification command/result.");
    }
    out
}
...
fn render_opencode_continue_prompt(
    ...
    if !transcript.is_empty() {
        ...
        for msg in compacted {
            ...
            out.push_str(&workspace_relative_prompt_text(
                workspace,
                msg.content.trim(),
            ));
            out.push_str("\n\n");
